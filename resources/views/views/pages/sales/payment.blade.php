...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('sales.paymentProccess')}}" method="POST">
                        @csrf
                        <div class="row">
                                ...
                                <table style="width: 100%;">
                                    ...
                                    <tbody>
                                        @foreach ($products as $product)
                                            <tr>
                                                <td>{{ $product['name'] }} <br> <small>{{ 'Rp. ' . number_format($product['price'], 0, ',', '.') }} X
                                                        {{ $product['quantity'] }}</small></td>
                                                <td><b>{{ 'Rp. ' . number_format($product['sub_total'], 0, ',', '.') }}</b>
                                                </td>
                                            </tr>
                                            <input type="hidden" name="shop[]" value="{{$product['product_id'].';'.$product['name'].';'.$product['price'].';'.$product['quantity'].';'.$product['sub_total']}}"
                                                hidden="">
                                        @endforeach
                                        <tr>
                                            <td style="padding-top: 20px; font-size: 20px;"><b>Total</b></td>
                                            <td class="tex-end" id="get_total" style="padding-top: 20px; font-size: 20px;">
    ...
    <script>
        ...
        function memberDetect(){
            ...
            const no_hp = document.getElementById('member-wrap');
            const is_member = detectElement.value;
            
            if (is_member == 'Member') {
                no_hp.classList.remove('d-none');
            }else{
                no_hp.classList.add('d-none');
            }
        }
